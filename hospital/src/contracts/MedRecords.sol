...
// // 0x771D3a7C2d6C59D9030731a48D60D8296303fC06
    ...
    function getRecord(uint pId, uint rId) public returns (uint[] memory cost, string[] memory procedure, address[] memory uploader, uint[] memory hospId, string[] memory docName,string[] memory image)
    {
        return (patients[pId].cost, patients[pId].procedure, patients[pId].uploader, patients[pId].hospId, patients[pId].doctorName, patients[pId].image );
    }
